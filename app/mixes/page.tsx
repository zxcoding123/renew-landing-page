...
export default function MixesPage() {
  ...
  const handleUseMix = (sounds: string[]) => {
    localStorage.setItem("renew-active-ids", JSON.stringify(sounds))
    if (sounds.length > 0) {
      localStorage.setItem("renew-theme-id", sounds[0])
    }
    router.push("/mix")
  }
    ...
    <>
      ...
      <section className="py-28 pt-40 bg-[#FAF9F6] min-h-screen relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            <h1 className="text-5xl md:text-6xl font-serif font-medium text-gray-900">
              Ready-Made Mixes
            </h1>
              ...
              >
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-[15px] mt-2">{mix.description}</p>
                  </div>
                </div>

                <div className="mt-auto pt-4">
                  <Button
                    onClick={() => handleUseMix(mix.sounds)}
                    className="w-full rounded-full bg-[#443737] text-white hover:bg-[#F6D2B5] hover:text-black transition-all duration-300 cursor-pointer"
                  >
                    Use Mix
                  </Button>
                </div>
